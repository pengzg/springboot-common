...
import xyz.carjoy.question.utils.BaseDataTypeCodeConstants;
...
public class BaseDataVO implements java.io.Serializable{
	...
	private String bd_state_nameref;
	//删除标识位
	...
	public String getBd_state_nameref() {
		if (StringUtils.isNotBlank(getBd_state()+"")) {
			bd_state_nameref = BaseDataDetailValueCache.getInstance().getName(BaseDataTypeCodeConstants.type_2006, getBd_state().toString());
		}
		return bd_state_nameref;
	}

	public void setBd_state_nameref(String bd_state_nameref) {
		this.bd_state_nameref = bd_state_nameref;
	}
}
